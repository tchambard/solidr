    ...
    <>
            ...
            {sessionCurrent.session?.status === SessionStatus.Opened && (
              <Tooltip placement={"bottom"} title={"Register new expense"}>
                <IconButton
                  color={"primary"}
                  onClick={() =>
                    setAddExpenseDialogVisible(!addExpenseDialogVisible)
                  }
                >
                  <AddCircleIcon />
                </IconButton>
      ...
      >
        {sessionCurrent?.expenses.map((expense) => {
          const expenseOwner = _.find(
            sessionCurrent?.members,
          ...
          return (
            <ListItem key={`expense_${expense.expenseId}`}>
              ...
              <ListItemAvatar>
                <AddressAvatar
                  key={`expense_voter_avatar-${expenseOwner.addr.toString()}`}
                  address={expenseOwner.addr.toString()}
                  size={24}
                />
              </ListItemAvatar>
            </ListItem>
      ...
      {addExpenseDialogVisible && (
        <SessionAddExpenseDialog
          dialogVisible={addExpenseDialogVisible}
          setDialogVisible={setAddExpenseDialogVisible}
        />
      )}
    </>
  );
...
const MyTotalCost: React.FC<{ totalCost: number | undefined }> = ({
  totalCost,
}) => {
  return <ListItemText primary={`My total cost ${totalCost ?? 0}€`} />;
};

const TotalExpenses: React.FC<{ totalExpenses: number | undefined }> = ({
  totalExpenses,
}) => {
